...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-auth-login': { kind: 'none', reason: 'Browser-side login presentation; registers nothing model-facing.' },
  ...
  'packages/session/session-search-projection-elasticsearch': { kind: 'none', reason: 'The Host-only search projection Consumer registers nothing model-facing; a search API owns query presentation.' },
  'packages/session/session-cdc-starter': { kind: 'none', reason: 'The Host-only CDC composition and reconciliation wiring register nothing model-facing.' },
