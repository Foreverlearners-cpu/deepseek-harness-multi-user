...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/session/session-message-change-protocol': { kind: 'none', reason: 'The pure Kafka wire codec registers nothing model-facing; projection Consumers own any downstream behavior.' },
  'packages/session/session-search-projection-elasticsearch': { kind: 'none', reason: 'The Host-only search projection Consumer registers nothing model-facing; a later search API owns query presentation.' },
