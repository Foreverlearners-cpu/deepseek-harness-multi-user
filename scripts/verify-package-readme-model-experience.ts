...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/identity/authentication': { kind: 'none', reason: 'Host-side identity verification and call metadata never enter a model request.' },
  'packages/identity/authentication-local': { kind: 'none', reason: 'The local identity Provider only supplies Host authorization metadata.' },
  'packages/identity/authorization': { kind: 'none', reason: 'Host-side permission decisions register no model context.' },
  'packages/identity/authorization-static': { kind: 'none', reason: 'The static policy Provider only evaluates Host authorization requests.' },
