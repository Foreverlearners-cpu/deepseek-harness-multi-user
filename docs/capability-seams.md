...
| `ctx.kafka` | `core` | [`kafka`](../packages/multi/kafka) | - | [`session-cache-invalidation-redis`](../packages/session/session-cache-invalidation-redis) | - | Owns one named Admin client, startup metadata verification, bounded health metadata, classified failures, and scoped shutdown; producer and consumer operations are deferred. |
...
| `ctx.redis` | `core` | [`redis`](../packages/multi/redis) | - | [`session-cache-invalidation-redis`](../packages/session/session-cache-invalidation-redis) | - | Owns one Host non-blocking client, disables offline queuing, and drains admitted callbacks; domain plugins own namespaces, TTLs, atomic commands, tenant scoping, and outage policy. |
