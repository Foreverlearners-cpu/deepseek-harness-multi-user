...
flowchart LR
  ...
  svc_elasticsearch["ctx.elasticsearch<br/>Elasticsearch search client"]
  pkg_redis["redis"]
  svc_redis["ctx.redis<br/>Redis command client"]
...
| `ctx.kafka` | `core` | [`kafka`](../packages/multi/kafka) | - | [`session-cache-invalidation-redis`](../packages/session/session-cache-invalidation-redis) | - | 拥有一个具名 Admin 客户端、启动元数据验证、有界健康元数据、分类错误和随 scope 关闭；producer 与 consumer 操作保持延后。 |
...
| `ctx.redis` | `core` | [`redis`](../packages/multi/redis) | - | [`session-cache-invalidation-redis`](../packages/session/session-cache-invalidation-redis) | - | 拥有一条 Host 非阻塞客户端，禁用离线排队，并等待已接纳回调；领域插件拥有 namespace、TTL、atomic command、tenant scoping 与故障策略。 |
