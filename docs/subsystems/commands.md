 ...
 */
@Remote({ access: 'authenticated' }) list(agent: Agent): readonly CommandDescriptor[]
 ...
 */
@Remote({ access: 'authenticated' }) async execute( agent: Agent, line: string, signal: AbortSignal, ): Promise<CommandExecution | undefined>
```
