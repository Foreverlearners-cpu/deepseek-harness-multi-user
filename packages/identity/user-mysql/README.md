...
User-scoped MySQL provider for `@deepseek-ai/dsh-user`. It owns the `dsh_users` table, keeps user lifecycle state separate from authentication credentials, and uses `@deepseek-ai/dsh-mysql` for connection and transaction ownership.
