...
Family creation writes the family and first credential in one transaction. Before commit, the Provider awaits the Host artifact preparation required by `dsh-auth-token`; a preparation failure rolls back both records. Rotation identifies the immutable family relation, locks the family before the submitted credential, consumes an active digest exactly once, and gives the replacement the locked family's absolute expiry. It also awaits preparation after staging those mutations and before commit, so failure rolls back the replacement and leaves the old credential active. A concurrent or later reuse locks the same rows, revokes the family and every active credential in that transaction, then returns `refresh-token-reused`. Credential, family, and principal revocation uses the same lock order and is idempotent.
