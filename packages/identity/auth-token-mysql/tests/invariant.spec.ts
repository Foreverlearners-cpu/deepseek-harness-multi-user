...
describe('auth-token-mysql invariant companion', () => {
  it('reserves package ownership without a runtime-only assertion', async () => {
    ...
    await expect(apply(ctx as never)).resolves.toBe(disposer)
    expect(name).toBe('auth-token-mysql-invariant')
    expect(inject).toEqual(['invariants'])
    expect(register).toHaveBeenCalledWith('@deepseek-ai/dsh-auth-token-mysql', expect.any(Function))
