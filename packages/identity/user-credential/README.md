...
`verifyPasswordSecret()` performs comparable password-verifier work for an unknown user, a user without a password, and an incorrect password. The public `verifyPassword()` returns `false` for all three cases, so callers cannot enumerate account or password state from its result. Unexpected Provider failures still become `provider-unavailable`; transports should apply generic messages and rate limits.

Expected storage failures use `UserCredentialError`: `identifier-conflict`, `identifier-not-found`, `password-not-set`, `invalid-credential`, and `revision-conflict`. The shared suite in `tests/contract.ts` is the normative Provider test and must run against every implementation.
