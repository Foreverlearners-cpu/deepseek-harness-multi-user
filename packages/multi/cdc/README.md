...
The producer uses `@deepseek-ai/dsh-kafka-events` for the typed CDC codec and per-event topic, key, and header routing. That runner performs one Kafka publish attempt and adds no retry or offset policy. CDC retains its serial queue, backpressure, bounded exponential publication retry, shutdown interruption, and checkpoint ordering.
