...
declare module '@deepseek-ai/dsh-typert-protocol' {
  ...
  interface TypertContextMap {
    ...
    gatewayFixtureAlias: TypertContext<string>
  }
}
...
class GoalService extends Service {
  ...
  @RemoteScope('gatewayFixture', { access: 'authenticated' })
...
const FIXTURE_READ_PERMISSION = permissionCode('fixture:read')

class ProtectedService extends Service {
  readonly typertRemote = bindTypertRemote(this, 'protectedFixture', { namespace: 'protected' })
  readonly calls: AuthenticatedCall[] = []

  constructor(ctx: Context) {
    super(ctx, 'protectedFixture')
  }

  @Remote({ exportName: 'read', permission: 'fixture:read' })
  read(call: AuthenticatedCall, value: string): string {
    this.calls.push(call)
    return `${call.principal.kind}:${value}`
  }
}

class ProtectedScopeService extends Service {
  readonly typertRemote = bindTypertRemote(this, 'protectedScopeFixture', { namespace: 'protected-scope' })
  readonly calls: Array<{ readonly call: AuthenticatedCall; readonly value: string; readonly scope: string }> = []

  constructor(ctx: Context) {
    super(ctx, 'protectedScopeFixture')
  }

  @RemoteScope('gatewayFixture', { exportName: 'read', permission: 'fixture:read' })
  read(call: AuthenticatedCall, value: string): string {
    const scope = (this.ctx as MarkedContext).fixtureScope ?? 'root'
    this.calls.push({ call, value, scope })
    return `${call.principal.kind}:${scope}:${value}`
  }
}

class ProtectedLookupService extends Service {
  readonly typertRemote = bindTypertRemote(this, 'protectedLookupFixture', { namespace: 'protected-lookup' })
  readonly calls: AuthenticatedCall[] = []

  constructor(ctx: Context) {
    super(ctx, 'protectedLookupFixture')
  }

  @Remote({ exportName: 'read', permission: 'fixture:read' })
  read(call: AuthenticatedCall, agent: FixtureAgent): string {
    this.calls.push(call)
    return agent.id
  }
}

class CompetingGoalService extends Service {
  readonly typertRemote = bindTypertRemote(this, 'competingGoals', { namespace: 'goals' })
  readonly calls: string[] = []

  constructor(ctx: Context) {
    super(ctx, 'competingGoals')
  }
...
class DeferredAuthorizationProvider extends AuthorizationProvider {
  readonly started: Promise<void>
  private readonly result: Promise<AuthorizationProviderEffect>
  private start!: () => void
  private finish!: (effect: AuthorizationProviderEffect) => void

  constructor(ctx: Context) {
    super(ctx)
    this.started = new Promise<void>((resolve) => { this.start = resolve })
    this.result = new Promise<AuthorizationProviderEffect>((resolve) => { this.finish = resolve })
  }

  allow(): void {
    this.finish({ effect: 'allow' })
  }
...
interface ExpiringAuthenticationConfig {
  readonly expiresAt: number
}

class ExpiringAuthenticationProvider extends AuthenticationProvider {
  constructor(ctx: Context, private readonly config: ExpiringAuthenticationConfig) {
    super(ctx)
  }

  protected verify(_attempt: AuthenticationAttempt): Promise<VerifiedAuthentication> {
    return Promise.resolve({
      principal: { kind: 'local', id: localPrincipalId('gateway-test') },
      method: authenticationMethod('expiring-fixture'),
      scope: {
        kind: 'tenant',
        tenantId: tenantId('gateway-tenant'),
        membershipId: membershipId('gateway-membership'),
      },
...
type FakeRpcHandler = (endpoint: string, payload: unknown, call: AuthenticatedCall) => Promise<FakeRpcResult>
...
describe('TypertGatewayService', () => {
  it('requires a Provider-owned identity even when a Remote has no action permission yet', async () => {
    const { ctx, service } = await setup()
    const call = await authenticatedCall(ctx)
  ...
  it('authorizes a protected SRC Remote before argument validation and injects the Host-only call', async () => {
    const ctx = await setupGateway()
    await ctx.plugin(ProtectedService)
    ctx.authorization.permissions.register({
      code: FIXTURE_READ_PERMISSION,
      owner: '@fixture/gateway',
      description: 'Read the protected Gateway fixture.',
      disclosure: 'metadata',
    })
    const call = await authenticatedCall(ctx)
    ...
    await expect(ctx.typertGateway.invoke({
      ...
      method: 'read',
      args: { value: 'visible' },
    })).resolves.toBe('local:visible')
    const receiver = ctx.get('protectedFixture') as unknown as ProtectedService & {
      [symbols.original]?: ProtectedService
    }
    const service = receiver[symbols.original] ?? receiver
  ...
  it('defaults protected Remotes to denial for unregistered actions and explicit deny-all policy', async () => {
    const unknown = await setupGateway()
    await unknown.plugin(ProtectedService)
    await expect(unknown.typertGateway.invoke({
      call: await authenticatedCall(unknown),
      namespace: 'protected',
      method: 'read',
      args: { value: 'hidden' },
    })).rejects.toMatchObject({
    ...
    await installSecurity(denied, 'deny-all')
    await denied.plugin(TypertRegistry)
    await denied.plugin(TypertGatewayService)
    await denied.plugin(ProtectedService)
    denied.authorization.permissions.register({
      code: FIXTURE_READ_PERMISSION,
      owner: '@fixture/gateway',
      description: 'Read the protected Gateway fixture.',
      disclosure: 'metadata',
    })
    await expect(denied.typertGateway.invoke({
      call: await authenticatedCall(denied),
      namespace: 'protected',
      method: 'read',
      args: { unexpected: true },
  ...
  it('authorizes permission RemoteScope before Context resolution and injects the Host call', async () => {
    const allowed = await setupGateway()
    await allowed.plugin(ProtectedScopeService)
    allowed.authorization.permissions.register({
      code: FIXTURE_READ_PERMISSION,
      owner: '@fixture/gateway',
      description: 'Read the protected Gateway fixture.',
      disclosure: 'metadata',
    })
    const scoped = allowed.extend({ fixtureScope: 'protected-agent' })
    let resolves = 0
    allowed.typert.contexts.registerHost('gatewayFixture', {
    ...
    const call = await authenticatedCall(allowed)

    await expect(allowed.typertGateway.invoke({
      call,
      namespace: 'protected-scope',
      method: 'read',
      args: { agentId: 'agent-1', value: 'visible' },
    })).resolves.toBe('local:protected-agent:visible')
    const receiver = allowed.get('protectedScopeFixture') as unknown as ProtectedScopeService & {
      [symbols.original]?: ProtectedScopeService
    }
    expect((receiver[symbols.original] ?? receiver).calls).toEqual([{ call, value: 'visible', scope: 'protected-agent' }])
    ...
    await denied.plugin(TypertRegistry)
    await denied.plugin(TypertGatewayService)
    await denied.plugin(ProtectedScopeService)
    denied.authorization.permissions.register({
      code: FIXTURE_READ_PERMISSION,
      owner: '@fixture/gateway',
      description: 'Read the protected Gateway fixture.',
    ...
    denied.typert.contexts.registerHost('gatewayFixture', {
      ...contextProvider(denied.extend()),
      resolve: () => {
        deniedResolves += 1
        return undefined
      },
    ...
    await expect(denied.typertGateway.invoke({
      ...
      namespace: 'protected-scope',
      method: 'read',
      args: { agentId: 'missing', unexpected: true },
    })).rejects.toBeInstanceOf(AuthorizationDeniedError)
    expect(deniedResolves).toBe(0)
  })

  it('refuses a permission RemoteScope when policy changes during Context resolution', async () => {
    const ctx = await setupGateway()
    await ctx.plugin(ProtectedScopeService)
    ctx.authorization.permissions.register({
      code: FIXTURE_READ_PERMISSION,
      owner: '@fixture/gateway',
      description: 'Read the protected Gateway fixture.',
    ...
    const scoped = ctx.extend({ fixtureScope: 'protected-agent' })
    let contextStarted!: () => void
    let finishContext!: (context: Context) => void
    const started = new Promise<void>((resolve) => { contextStarted = resolve })
    const result = new Promise<Context>((resolve) => { finishContext = resolve })
    ...
    const pending = ctx.typertGateway.invoke({
      ...
      namespace: 'protected-scope',
      method: 'read',
      args: { agentId: 'agent-1', value: 'hidden' },
    })
    await started
    ctx.authorization.permissions.register({
      code: permissionCode('fixture:write'),
      owner: '@fixture/gateway',
      description: 'Mutate the protected Gateway fixture.',
      disclosure: 'administration',
    })
    finishContext(scoped)

    await expect(pending).rejects.toMatchObject({
      name: 'AuthorizationDeniedError',
      reason: 'policy-stale',
    })
    const receiver = ctx.get('protectedScopeFixture') as unknown as ProtectedScopeService & {
      [symbols.original]?: ProtectedScopeService
    }
    expect((receiver[symbols.original] ?? receiver).calls).toEqual([])
  })

  it('does not let a tampered strict descriptor downgrade a permission RemoteScope', async () => {
    const ctx = await setupGateway()
    await ctx.plugin(ProtectedScopeService)
    const descriptor = protectedScopeDescriptor()
    registerStrict(ctx, [descriptor])
    const mutable = descriptor as unknown as { access: string; authorization?: object }
    mutable.access = 'authenticated'
    delete mutable.authorization
    let resolves = 0
    ctx.typert.contexts.registerHost('gatewayFixture', {
    ...
    await expectCode(ctx.typertGateway.invoke({
      call: await authenticatedCall(ctx),
      namespace: 'protected-scope',
      method: 'read',
      args: { agentId: 'missing', value: 'hidden' },
    }), 'binding-invalid')
    expect(resolves).toBe(0)
  })

  it.each(['direct', 'wrong-context'] as const)(
    'rejects a tampered strict RemoteScope %s invocation before Context resolution',
    async (variant) => {
      const ctx = await setupGateway()
      await ctx.plugin(ProtectedScopeService)
      const descriptor = protectedScopeDescriptor()
      registerStrict(ctx, [descriptor])
      const originalInvocation = descriptor.invocation
      if (originalInvocation.kind !== 'context') throw new Error('fixture descriptor must be Context-scoped')
      const mutable = descriptor as unknown as { invocation: InvocationDescriptor['invocation'] }
      mutable.invocation = variant === 'direct'
        ? { kind: 'direct' }
        : { ...originalInvocation, context: 'gatewayFixtureAlias' }
      let resolves = 0
      ctx.typert.contexts.registerHost('gatewayFixtureAlias', {
        ...contextProvider(ctx.extend()),
        resolve: () => {
          resolves += 1
          return ctx.extend()
  ...
  it('refuses a protected Remote when policy changes during an asynchronous lookup', async () => {
    const ctx = await setupGateway()
    await ctx.plugin(ProtectedLookupService)
    ctx.authorization.permissions.register({
      code: FIXTURE_READ_PERMISSION,
      owner: '@fixture/gateway',
      description: 'Read the protected Gateway fixture.',
    ...
    let finishLookup!: (agent: FixtureAgent) => void
    const started = new Promise<void>((resolve) => { lookupStarted = resolve })
    const resolved = new Promise<FixtureAgent>((resolve) => { finishLookup = resolve })
    ...
    finishLookup({ id: 'agent-1' })

    await expect(pending).rejects.toMatchObject({
      name: 'AuthorizationDeniedError',
      reason: 'policy-stale',
    })
    const receiver = ctx.get('protectedLookupFixture') as unknown as ProtectedLookupService & {
      [symbols.original]?: ProtectedLookupService
    }
    expect((receiver[symbols.original] ?? receiver).calls).toEqual([])
  })

  it('refuses a protected Remote when its Service is replaced during authorization evaluation', async () => {
    const ctx = new Context()
    await ctx.plugin(LocalAuthenticationProvider, {
      principalId: 'gateway-test',
      tenantId: 'gateway-tenant',
      membershipId: 'gateway-membership',
    })
    await ctx.plugin(DeferredAuthorizationProvider)
    await ctx.plugin(TypertRegistry)
    await ctx.plugin(TypertGatewayService)
    const serviceFiber = ctx.plugin(ProtectedService)
    await serviceFiber
    ctx.authorization.permissions.register({
      code: FIXTURE_READ_PERMISSION,
      owner: '@fixture/gateway',
    ...
    const oldReceiver = ctx.get('protectedFixture') as unknown as ProtectedService & {
      [symbols.original]?: ProtectedService
    }
    const oldService = oldReceiver[symbols.original] ?? oldReceiver
    const authorizationReceiver = ctx.get('authorization') as unknown as DeferredAuthorizationProvider & {
      [symbols.original]?: DeferredAuthorizationProvider
    }
    const authorization = authorizationReceiver[symbols.original] ?? authorizationReceiver
    const pending = ctx.typertGateway.invoke({
      call: await authenticatedCall(ctx),
      namespace: 'protected',
      method: 'read',
    ...
    await ctx.plugin(ProtectedService)
    const replacementReceiver = ctx.get('protectedFixture') as unknown as ProtectedService & {
      [symbols.original]?: ProtectedService
    }
    const replacement = replacementReceiver[symbols.original] ?? replacementReceiver
    authorization.allow()

    await expectCode(pending, 'service-unavailable')
    expect(oldService.calls).toEqual([])
    expect(replacement.calls).toEqual([])
  })

  it('refuses a strict invocation withdrawn during an asynchronous lookup', async () => {
    const { ctx, service } = await setup()
    let lookupStarted!: () => void
    let finishLookup!: (agent: FixtureAgent) => void
    const started = new Promise<void>((resolve) => { lookupStarted = resolve })
    ...
    const disposeDefinition = registerStrict(ctx, [createDescriptor()])
    ...
    await expectCode(pending, 'definition-unavailable')
    expect(service.calls).toEqual([])
  })

  it('refuses lookup and Context results from providers replaced while pending', async () => {
    const lookupCase = await setup()
    let lookupStarted!: () => void
    let finishLookup!: (agent: FixtureAgent) => void
    const lookupPending = new Promise<void>((resolve) => { lookupStarted = resolve })
    const lookupResult = new Promise<FixtureAgent>((resolve) => { finishLookup = resolve })
    const disposeLookup = lookupCase.ctx.typert.lookups.register('gatewayFixture', {
      ...agentLookup({ id: 'agent-1' }),
      resolve: async () => {
        lookupStarted()
        return lookupResult
    ...
    registerStrict(lookupCase.ctx, [createDescriptor()])
    const pendingLookup = invoke(lookupCase.ctx, {
      namespace: 'goals',
      method: 'create',
      args: { agentId: 'agent-1', request: { title: 'ship' } },
    })
    await lookupPending
    await disposeLookup()
    registerAgentLookup(lookupCase.ctx, { id: 'agent-1' })
    finishLookup({ id: 'agent-1' })
    await expectCode(pendingLookup, 'provider-mismatch')
    expect(lookupCase.service.calls).toEqual([])

    const contextCase = await setup()
    const oldScope = contextCase.ctx.extend({ fixtureScope: 'old-scope' })
    const newScope = contextCase.ctx.extend({ fixtureScope: 'new-scope' })
    let contextStarted!: () => void
    let finishContext!: (scope: Context) => void
    const contextPending = new Promise<void>((resolve) => { contextStarted = resolve })
    const contextResult = new Promise<Context>((resolve) => { finishContext = resolve })
    const disposeContext = contextCase.ctx.typert.contexts.registerHost('gatewayFixture', {
      ...contextProvider(oldScope),
      resolve: async () => {
        contextStarted()
        return contextResult
      },
    })
    registerStrict(contextCase.ctx, [renameDescriptor()])
    const pendingContext = invoke(contextCase.ctx, {
      namespace: 'goals',
      method: 'rename',
    ...
    contextCase.ctx.typert.contexts.registerHost('gatewayFixture', contextProvider(newScope))
    finishContext(oldScope)
    await expectCode(pendingContext, 'provider-mismatch')
    expect(contextCase.service.calls).toEqual([])
  })

  it('revalidates Authentication Provider ownership after an asynchronous lookup', async () => {
    const ctx = new Context()
    const authentication = ctx.plugin(LocalAuthenticationProvider, {
      principalId: 'gateway-test',
      tenantId: 'gateway-tenant',
      membershipId: 'gateway-membership',
    })
    await authentication
    await ctx.plugin(StaticAuthorizationProvider, { mode: 'trusted-local' })
    await ctx.plugin(TypertRegistry)
    await ctx.plugin(TypertGatewayService)
    ...
    let finishLookup!: (agent: FixtureAgent) => void
    const started = new Promise<void>((resolve) => { lookupStarted = resolve })
    const result = new Promise<FixtureAgent>((resolve) => { finishLookup = resolve })
    ctx.typert.lookups.register('gatewayFixture', {
      ...agentLookup({ id: 'agent-1' }),
      resolve: async () => {
        lookupStarted()
    ...
    await expect(pending).rejects.toMatchObject({ name: 'AuthenticationError', code: 'unauthenticated' })
    expect(rawGoalService(ctx).calls).toEqual([])
  })

  it('revalidates credential expiry after an asynchronous lookup', async () => {
    const ctx = new Context()
    const expiresAt = Date.now() + 60_000
    await ctx.plugin(ExpiringAuthenticationProvider, { expiresAt })
    await ctx.plugin(StaticAuthorizationProvider, { mode: 'trusted-local' })
    await ctx.plugin(TypertRegistry)
    await ctx.plugin(TypertGatewayService)
    await ctx.plugin(GoalService)
  ...
  it('refuses an SRC invocation when a second claimant appears during lookup', async () => {
    const { ctx, service } = await setup()
    let lookupStarted!: () => void
    let finishLookup!: (agent: FixtureAgent) => void
    const started = new Promise<void>((resolve) => { lookupStarted = resolve })
    ...
    await started
    await ctx.plugin(CompetingGoalService)
    const competingReceiver = ctx.get('competingGoals') as unknown as CompetingGoalService & {
      [symbols.original]?: CompetingGoalService
    }
    const competing = competingReceiver[symbols.original] ?? competingReceiver
    finishLookup({ id: 'agent-1' })

    await expectCode(pending, 'ambiguous-endpoint')
    expect(service.calls).toEqual([])
    expect(competing.calls).toEqual([])
  })

  it('refuses an SRC invocation when its owning Service is reloaded during lookup', async () => {
    ...
    await serviceFiber.dispose()
    await ctx.plugin(GoalService)
    const replacement = rawGoalService(ctx)
    finishLookup({ id: 'agent-1' })
  ...
  it('rejects SRC wire collisions and unavailable Context providers', async () => {
    ...
    await expectCode(invoke(colliding, {
    ...
    await expectCode(invoke(missing.ctx, {
      namespace: 'goals',
      method: 'rename',
      args: { agentId: 'agent-1', request: { title: 'land' } },
    }), 'context-unavailable')
    ...
    await expectCode(invoke(contextCollision, {
  ...
  it('re-reads and contains Context providers', async () => {
    ...
    const error = await expectCode(invoke(ctx, {
      namespace: 'goals',
      method: 'rename',
      args: { agentId: 'agent-1', request: { title: 'land' } },
    }), 'context-failed')
  ...
  it('contains lookup provider failures and missing identities', async () => {
    ...
    const failure = await expectCode(invoke(ctx, {
      namespace: 'goals',
      method: 'create',
      args: { agentId: 'agent-1', request: { title: 'ship' } },
    }), 'lookup-failed')
  ...
  it('requires a visible binding and a live Remote marker for explicitly provided Services', async () => {
    const ctx = await setupGateway()
  ...
  it('mounts a shared /api interceptor through an optional Connection and returns existing RPC results', async () => {
    ...
    await expect(handler('goals/create', {
      args: { agentId: 'agent-1', request: { title: 'ship' } },
    }, await authenticatedCall(ctx, signal))).resolves.toEqual({
      ok: true,
    ...
    const invalid = await handler('goals/create', { invalid: true }, await authenticatedCall(ctx, signal))
    expect(invalid).toMatchObject({
      ok: false,
      error: { code: 'internal' },
    })
    if (invalid.ok) throw new Error('invalid Remote payload unexpectedly succeeded')
    expect(invalid.error.message).toBe('handler failure')

    await expect(handler('goals/maybe', { args: {} }, await authenticatedCall(ctx, signal))).resolves.toEqual({
      ok: true,
      value: undefined,
    })
    await expect(handler(
      'goals/maybe',
      { args: { value: null } },
      await authenticatedCall(ctx, signal),
    )).resolves.toEqual({
      ok: true,
      value: null,
    })
    ...
    for (const endpoint of ['goals', '/create', 'goals/', 'goals/create/extra']) {
      const result = await handler(endpoint, { args: {} }, await authenticatedCall(ctx, signal))
    ...
    for (const payload of [null, [], { args: {}, extra: true }, { only: true }, { args: null }, { args: [] }]) {
      const result = await handler('goals/create', payload, await authenticatedCall(ctx, signal))
      expect(result).toMatchObject({ ok: false, error: { code: 'internal' } })
    ...
    await expect(handler(
      'goals/fail',
      { args: { request: null } },
      await authenticatedCall(ctx, cancelledCall.signal),
    )).resolves.toEqual({
      ok: false,
      error: {
        code: 'cancelled',
  ...
  it('maps protected RPC refusals without exposing internal authorization reasons', async () => {
    async function protectedHandler(mode: 'deny-all' | 'trusted-local'): Promise<{
      readonly ctx: Context
      readonly handler: FakeRpcHandler
    }> {
      const ctx = new Context()
      await installSecurity(ctx, mode)
      await ctx.plugin(TypertRegistry)
      await ctx.plugin(FakeConnectionService)
      await ctx.plugin(TypertGatewayService)
      await ctx.plugin(ProtectedService)
    ...
    const forbidden = await protectedHandler('deny-all')
    await expect(forbidden.handler(
      'protected/read',
      { args: { value: 'hidden' } },
      await authenticatedCall(forbidden.ctx),
    )).resolves.toEqual({
      ok: false,
      error: {
        code: 'permission-denied',
        message: 'authorization denied for fixture:read',
        details: { permission: 'fixture:read' },
      },
    })

    const unauthenticated = await protectedHandler('trusted-local')
    const valid = await authenticatedCall(unauthenticated.ctx)
    await expect(unauthenticated.handler(
      'protected/read',
      { args: { value: 'hidden' } },
      { ...valid },
    )).resolves.toEqual({
      ...
      error: {
        code: 'unauthenticated',
        message: 'authenticated call is invalid',
        details: {},
      },
    })
  })
  ...
  it('preserves a lookup policy rejection through the Connection RPC result', async () => {
    ...
    await expect(handler('goals/create', {
      args: { agentId: 'agent-1', request: { title: 'ship' } },
    }, await authenticatedCall(ctx))).resolves.toEqual({ ok: false, error: failure })
  })
  ...
  it('dispatches claimed invocations through /api and leaves unclaimed endpoints to its fallback', async () => {
    ...
    try {
      ...
      expect(JSON.stringify(invalidBody)).not.toContain('plain-object args field')
      expect(JSON.stringify(invalidBody)).toContain('handler failure')
      ...
      expect(JSON.stringify(withdrawnBody)).not.toContain('strict definition was withdrawn')
      expect(JSON.stringify(withdrawnBody)).toContain('handler failure')
...
async function installSecurity(
  ctx: Context,
  mode: 'deny-all' | 'trusted-local' = 'trusted-local',
): Promise<void> {
  await ctx.plugin(LocalAuthenticationProvider, {
    principalId: 'gateway-test',
    tenantId: 'gateway-tenant',
...
async function authenticatedCall(
  ...
  return ctx.authentication.authenticate({
    requestId: authenticationRequestId(`gateway-test-${String(authenticationRequestSequence)}`),
    channel: 'in-process',
...
type TestInvokeRemoteRequest = Omit<InvokeRemoteRequest, 'call'> & { readonly signal?: AbortSignal }

async function invoke(ctx: Context, request: TestInvokeRemoteRequest): Promise<unknown> {
  const { signal, ...remote } = request
  const call = await authenticatedCall(ctx, signal)
  return ctx.typertGateway.invoke({ ...remote, call })
}
...
function protectedScopeDescriptor(): InvocationDescriptor {
  const value = strictCodec('@fixture/gateway#ProtectedScopeValue', z.string())
  return {
    id: '@fixture/gateway#protected-scope/read',
    service: 'protectedScopeFixture',
    namespace: 'protected-scope',
    method: 'read',
    access: 'permission',
    ...
    parameters: [{ name: 'value', wire: 'value', source: 'json', codec: value }],
    result: value,
  }
}
