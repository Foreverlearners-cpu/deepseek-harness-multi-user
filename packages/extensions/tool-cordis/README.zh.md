...
`src/inspect.ts` 把这份目录与**活的**服务存储取交集：**谁在跑**由存储回答，**每个服务能做什么**由目录回答。生成的 `MODEL_HIDDEN_SERVICE_KEYS` 会在渲染任何实时事实前移除可信基础设施；目录没覆盖到的其他活服务会被报成正在运行但没有签名，而不是被省略。包代码若要在自己源码里使用可见清单，就从报告里复制——目录是关于仓库的编译期事实，所以对任一个部署而言，复制的清单与即时读取的清单说的是同一件事。
...
- **只向模型点名模型可达的键。** 反射模型覆盖每个已声明的 `ctx.<key>`，包括 launcher 值、其他 face 的服务和可信 Host 基础设施。生成器会从 `SERVICE_API` 排除模型编写包无法使用的键，并把它们记录到 `MODEL_HIDDEN_SERVICE_KEYS`；服务、API、动态包 provider 和缺失注入报告都会在渲染前过滤该集合。`verify-cordis-catalog` 证明每个隐藏键仍保留在维护者文档中，同时不进入模型报告。

生成常量 `INHERITED_CTX_API` 为 `api` 报告收尾，列出框架继承来的 `ctx` 面（`ctx.on`、`ctx.effect`、`ctx.loader`、各 timer 辅助方法）：这些成员本身就是 Context，不是某个服务键；而框架层住在 pinned vendor 包里，位于每一个被分析的契约面之外——所以生成器策展这**一层**，并把它同时渲染进本目录与 `docs/cordis-api/inherited.md`。一个可见但目录未描述的活服务会被报成正在运行但没有签名。宽泛的 `api`／`events` 报告只渲染摘要与签名；精确 `name` 会选择保留的方法／事件 JSDoc，未知、隐藏或未运行的服务目标会高声失败。
