...
`src/inspect.ts` intersects that catalog with the LIVE service store: what is RUNNING comes from the store and what each service CAN DO comes from the catalog. The generated `MODEL_HIDDEN_SERVICE_KEYS` removes trusted infrastructure before any live fact is rendered; another live service the catalog does not cover is reported with no signatures rather than omitted. A package that needs the visible list in its own code copies it out of a report — the catalog is a compile-time fact about the repository, so a copied list and a freshly read one say the same thing for any one deployment.
...
- **Only model-reachable keys are named.** The reflection model covers every declared `ctx.<key>`, including launcher values, other-face services, and trusted Host infrastructure. The generator omits keys that a model-authored package cannot use from `SERVICE_API` and records them in `MODEL_HIDDEN_SERVICE_KEYS`; service, API, dynamic-package provider, and missing-injection reports filter that set before rendering. `verify-cordis-catalog` proves each hidden key remains in maintainer documentation while staying out of model reports.

The generated `INHERITED_CTX_API` closes the `api` report with the framework-inherited `ctx` surface (`ctx.on`, `ctx.effect`, `ctx.loader`, the timer helpers): those members are the Context itself rather than service keys, and the framework tier lives in pinned vendor packages outside every analyzed face, so the generator curates that one tier and renders it into both this catalog and `docs/cordis-api/inherited.md`. A visible live service the catalog does not describe is reported as running without signatures. Broad `api` / `events` reports render summaries and signatures only; an exact `name` opts into the retained method/event JSDoc, and unknown, hidden, or non-running service targets fail loud.
