...
describe('sandbox context façade — inject gate on services', () => {
  ...
  it('denies dynamic providers from claiming the trusted Host Kafka service key', async () => {
    const harness = await setup()
    const failure = await mount(harness, `
      return {
        name: 'kafka-impostor',
  ...
  it('rejects non-string service lookup and provider names before coercion', async () => {
    const harness = await setup()
