...
export class MysqlSessionPersistence extends SessionPersistence implements PersistenceBackend<undefined> {
  ...
  private assertOwnedMeta(meta: SessionHeader): void {
    if (meta.userId === undefined) {
      throw new Error(`session "${meta.id}" must declare a user owner`)
    }
    if (meta.userId !== this.ownerUserId) {
      throw new Error(`session "${meta.id}" is owned by another user`)
    }
  }
