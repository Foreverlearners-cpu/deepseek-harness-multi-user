...
| `maxBatchRecords` | `64` | 触发写入的记录数，也是单批最大记录数 |
| `maxBatchBytes` | `524288` | 按完整记录 UTF-8 大小触发写入，也是单批最大字节数；单条超限记录会独立写入 |
...
一个 Session 事件可以在一次原子 append 中生成一组相邻且共享 `sourceSequence` 的记录。Provider 必须原子提交整组记录。绑定时插件扫描已有记录；只要该 `sourceSequence` 已存在，就删除整组待写记录。
...
- 工具结果仍然内联保存。后续对象存储策略会把超过 256 KiB 的结果替换为文件引用。
