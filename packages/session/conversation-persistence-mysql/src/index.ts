...
export class MysqlConversationPersistence extends Service {
    ...
    this.spool = new DurableMessageSpool(
      spoolRoot, String(this.userId), config.spoolRetryMs ?? 5000,
        task => this.persistTurn({
          id: SessionId(task.sessionId), version: SESSION_FORMAT_VERSION,
  ...
  /** Convert only durable surface messages and title events; raw chunks are ignored. */
    private async handleSessionEvent(session: Session, event: SessionEvent): Promise<void> {
      ...
      if (rawEvent.type === 'session/title') {
        if (await this.getConversation(String(session.id)) === undefined) return
  ...
  async listAttempts(sessionId: string): Promise<ModelAttempt[]> {
    ...
    private async ensureConversation(header: SessionHeader): Promise<ConversationRecord> {
    const sessionId = String(header.id)
   ...
   */
  async listConversations(options: { includeDeleted?: boolean; limit?: number; offset?: number } = {}): Promise<ConversationRecord[]> {
    await this.ready
  ...
  /**
   * Commit final messages and model attempts in one transaction. The operation
   * is idempotent by message id: a retry that sees the complete batch already
   * present returns without creating duplicate attempts. Streaming chunks
  ...
  async updateTitle(
    ...
    await this.ctx.mysql.connection(async (connection) => {
      ...
      try {
        ...
        if (row.title_status === 'manual' && status !== 'manual') return await connection.commit()
        const now = Date.now()
        await connection.query(
   ...
   * @returns The updated conversation row.
   */
  async updateExtensions(sessionId: string, extensions: JsonObject, expectedRevision?: number): Promise<ConversationRecord> {
    await this.ready
