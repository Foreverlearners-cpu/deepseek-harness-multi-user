...
This plugin is the ToC conversation read/write model. It owns the user-scoped MySQL conversation projection; the canonical event log remains with a `SessionPersistence` provider such as `@deepseek-ai/dsh-session-persistence-mysql` for deployments that need lossless event-log replay.
...
- The same transaction records model attempts for the projection. Semantic outbox fan-out is intentionally out of scope in this release.
...
The plugin contributes no prompt or tool schema. It projects final `user/message`, `assistant/message`, and `tool/result` rows into MySQL while keeping streaming chunks out of the durable message projection.

#### Token effect

Zero direct tokens during normal operation; persistence and retry metadata are host-side. An interrupted generation is stored as a `partial` or `failed` assistant message; no chunk row is required for recovery.
