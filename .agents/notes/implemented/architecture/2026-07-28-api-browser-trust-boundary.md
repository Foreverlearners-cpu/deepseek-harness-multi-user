...
Two boundaries stay deliberately separate: reachability is the webserver binding's policy (`host: 127.0.0.1 | 0.0.0.0`), while identity and permissions are enforced by the configured Authentication/Authorization Providers in `dsh-client-connection`. The fence remains a confused-deputy defense, not an auth layer. The production `/api` mount authenticates and authorizes unary routes, `respond`, session export, SSE streams, and WebSocket upgrades; a local-only Provider cannot be combined with non-empty `trustedHosts`, and remote deployments must provide a network-capable Provider. The old guard's loopback-socket check was dropped rather than generalized: with binding expressing reachability and `trustedHosts` naming remote authorities, the socket address adds nothing a header fence does not already cover.
...
- A `0.0.0.0` deployment must declare serving authorities and use a network-capable Authentication Provider; the local bootstrap Provider is rejected for that composition. The trusted-network fence and application identity are separate checks.
