...
每个 tenant/user/session 都有一个 cache key 和独立 revision watermark key。一个 Redis Lua operation 比较规范十进制 revision，只推进更新的 watermark 并删除 cache。cache population 使用导出的 Lua-backed `refillSessionContextCache()` helper，只在权威 revision 不低于 watermark 时写入。这两个 operation 彼此保持原子性。
...
重复和乱序 CDC record 根据权威 revision 保持幂等，tenant 不会共享 cache key，旧 refill attempt 无法覆盖当前 cache state。消费方必须使用导出的 refill helper，revision 必须是规范正十进制整数，任何 row 或 schema change 都需要协调配置和代码变更。
