...
CDC composes `KafkaEventProducer<CdcEvent>` with its strict protocol codec and a router that derives the configured topic, compound primary-key bytes, and protocol headers. The generic runner makes exactly one transport call and does not own retry or offset decisions. CDC keeps publication retry, reader pause and resume, queue admission, shutdown interruption, and checkpoint advancement in its own lifecycle.
