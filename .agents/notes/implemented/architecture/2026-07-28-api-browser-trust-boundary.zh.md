...
两条边界刻意分开：可达性由 webserver 的绑定配置（`host: 127.0.0.1 | 0.0.0.0`）控制，身份与权限则由 `dsh-client-connection` 配置的 Authentication／Authorization Provider 执行。这道栅栏仍是混淆代理人防御，不是认证层。生产 `/api` mount 会认证并授权 unary route、`respond`、session export、SSE 流和 WebSocket upgrade；local-only Provider 不能与非空 `trustedHosts` 组合，远程部署必须提供支持网络的 Provider。旧守卫的回环 socket 检查被放弃而非泛化：绑定表达可达性、`trustedHosts` 点名远程权威之后，socket 地址提供不了头部栅栏覆盖不到的任何东西。
...
- `0.0.0.0` 部署必须声明服务 authority 并使用支持网络的 Authentication Provider；local bootstrap Provider 会被拒绝用于该组合。可信网络栅栏与应用身份是两道独立检查。
